...
from utils.data import iCIFAR10, iCIFAR100, iImageNet100, iImageNet1000, iTHUPIDatasets, iTHUPI30Datasets, iTHU_SEED_PIDatasets

class DataManager(object):
    def __init__(self, dataset_name, shuffle, seed, init_cls, increment, args):
        self.dataset_name = dataset_name
        self._setup_data(dataset_name, shuffle, seed, args)
        assert init_cls <= len(self._class_order), "No enough classes."
        self._increments = [init_cls]
        if args["cls_model"] == "joint":
            logging.info("Joint Training No Increments")
        else:
    ...
    def get_dataset(
        ...
        if ret_data:
            ...
            return data, targets, PIDataset(data,
                                            targets)  # DummyDataset(data, targets, trsf, self.use_path) -> TensorDataset(data, targets)
        else:
            data = torch.from_numpy(data).float()
            targets = torch.from_numpy(targets).long()
            return PIDataset(data,
                             targets)  # DummyDataset(data, targets, trsf, self.use_path) -> TensorDataset(data, targets)
    ...
    def _setup_data(self, dataset_name, shuffle, seed, args):
        idata = _get_idata(dataset_name, args)
        idata.download_data(args)
...
def _get_idata(dataset_name, args):
    ...
    if name == "cifar10":
        ...
        return iTHUPIDatasets(args)
    elif name == 'thupi30':
        return iTHUPI30Datasets(args)
    elif name == 'thu_seed_pi':
        return iTHU_SEED_PIDatasets(args)
    else:
