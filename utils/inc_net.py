...
def get_backbone(args, pretrained=False):
    ...
    if name == "resnet32":
        ...
        return CTFAT(input_size=(375, 6), patch_size=(15, 1), num_classes=10, dim=512, depth=8, heads=4,
                     mlp_dim=256, dropout=0.1, emb_dropout=0.1)
    elif name == 'mdcat':
        if args['frequency'] == 'all':
            return MD_CAT(input_size=(375, 6), patch_size=(15, 1), num_classes=10, dim=512, depth=8, heads=4,
                          mlp_dim=256, dropout=0.1, emb_dropout=0.1)
        else:
            return MD_CAT(input_size=(375, 1), patch_size=(15, 1), num_classes=10, dim=512, depth=8, heads=4,
                          mlp_dim=256, dropout=0.1, emb_dropout=0.1)
    elif name == 'mdcat_joint':
        return MD_CAT_Jointed(input_size=(375, 6), patch_size=(15, 1), num_classes=10, dim=512, depth=8, heads=4,
                              mlp_dim=256, dropout=0.1, emb_dropout=0.1)
    elif name == 'cat':
        return CAT(input_size=(375, 6), patch_size=(15, 1), num_classes=10, dim=512, depth=8, heads=4,
                   mlp_dim=256, dropout=0.1, emb_dropout=0.1)
    elif name == 'tfat':
        return TFAT(input_size=(375, 6), patch_size=(15, 1), num_classes=10, dim=512, depth=8, heads=4,
                    mlp_dim=256, dropout=0.1, emb_dropout=0.1)
    elif name == 'transformer':
        return OriTransformer(input_size=(375, 6), patch_size=(15, 1), num_classes=10, dim=512, depth=8, heads=4,
                              mlp_dim=256, dropout=0.1, emb_dropout=0.1)

    elif name == 'aitst':
        return AITST(input_size=(32, 375, 6), patch_size=(15, 1), channels=32, num_classes=10, model_dim=512,
                     pool='cls', dim_head=512, channel_rate=4,
                     depth=2, heads=2, mlp_dim=256, dropout=0.1, emb_dropout=0.1, model_structure="sa+ca")

    elif name == 'cit_net':
        return CIT_NET(in_channels=32, patch_size_x=15, patch_size_y=6, img_size=(375, 6))
    elif name == 'tsception':
        if args['frequency'] == 'all':
            return TSception(num_classes=2, input_size=(6, 32, 375), sampling_rate=125, num_T=32, num_S=256, hidden=512,
                             dropout_rate=0.5, fre=6, out_dim=256)
        else:
            return TSception(num_classes=2, input_size=(1, 32, 375), sampling_rate=125, num_T=32, num_S=256,
                             hidden=512,
                             dropout_rate=0.5, fre=1, out_dim=256)
    elif name == 'lstm':
        return LSTM(input_size=192, hidden_size=512, num_layers=3, num_classes=10)
    elif name == 'cbam_bilstm':
        return CBAM_BiLSTM(out_dim=256)
    elif name == 'gtnet':
        return GTNet(lstm_hidden_dim=256, lstm_num_layers=2, embed_dim=192, num_classes=10, dropout=0.1)
    elif name == 'bigru':
        return BiGRU(hidden_dim=128, num_layers=3, embed_dim=32, num_classes=10, dropout=0)
    elif name == 'cnn_gru':
        return CGRU(hidden_dim=192, num_layers=3, embed_dim=192, conv_channel=16, kernel_size=17, num_classes=10,
                    channel=32, dropout=0.1)
    elif name == 'cnn_lstm':
        return CLSTM(hidden_dim=192, num_layers=3, embed_dim=192, conv_channel=64, kernel_size=31, num_classes=10,
                     channel=375, dropout=0.1)
    elif name == 'mlp':
        return MLP(input_size=192, hidden_size1=512, hidden_size2=1024, output_size=512)
    elif name == 'cadcnn':
        return CADCNN(growthRate=16, depth=32, reduction=0.4,
                      bottleneck=True, nClasses=10, dropout_rate=0.1)
    elif name == 'dcnn':
        return DCNN(growthRate=16, depth=16, reduction=0.4,
                    bottleneck=True, nClasses=10, dropout_rate=0.1)
...
class JointedNet(BaseNet):
    def __init__(self, args, pretrained, gradcam=False):
        super().__init__(args, pretrained)
        self.gradcam = gradcam
        self.loss_weight = nn.Parameter(torch.tensor(-2.0), requires_grad=True)
    ...
    def forward(self, x):
        out = {}
        out["logits"] = self.backbone(x)["features"]
        return out
...
class DERNet(nn.Module):
    ...
    def update_fc(self, nb_classes):
        if len(self.convnets) == 0:
            self.convnets.append(get_backbone(self.args))
        else:
            self.convnets.append(get_backbone(self.args))
...
class AdaptiveNet(nn.Module):
    ...
    def update_fc(self, nb_classes):
        _, _new_extractor = get_backbone(self.args)
