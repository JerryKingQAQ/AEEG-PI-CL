...
class ChannelTimeFrequencyAttention(nn.Module):
...
class CTFAEncoder(nn.Module):
    def __init__(self, depth, channels, dim, hidden_dim):
        super().__init__()
        self.layers = nn.ModuleList([])
        for _ in range(depth):
            self.layers.append(nn.ModuleList([
                ChannelTimeFrequencyAttention(channel=channels),
...
if __name__ == '__main__':
    ...
    model = CTFAEncoder(depth=4, channels=32, dim=6, hidden_dim=256)
    out = model(x)
    print(out.shape)
